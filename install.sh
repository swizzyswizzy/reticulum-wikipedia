...
REPO="${1:-${RNS_GW_REPO:-git@github.com:swizzyswizzy/reticulum-wikipedia.git}}"
...
chmod 700 /root/.ssh
KEY=/root/.ssh/github
for src in /boot/firmware/rns_deploy_key /boot/rns_deploy_key /root/.ssh/github /root/.ssh/id_ed25519 /root/.ssh/id_rsa; do
  [ -s "$src" ] || continue
